"""
models.py - 极简 4D 密度场（时间作为输入）
使用 4D 哈希网格（若 tinycudann 可用）或 3D 哈希 + 时间位置编码，
直接输出密度值。
完全移除变形场、速度场、生长率等一切中间变量。
"""
...
class TemporalDensityField(nn.Module):
    ...
    def __init__(
        ...
        if self.use_hash:
            ...
            time_in_dim = 1 * (2 * num_time_freqs + 1)  # 1 * 13 = 13 (if 6)
...
# 为兼容旧代码，保留一个别名
CanonicalNerfField = TemporalDensityField