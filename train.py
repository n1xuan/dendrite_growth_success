...
损失：渲染损失 + 时间平滑 + 质量守恒 + 可选单调性 + 空间 TV (抗模糊)
"""
...
class TrainConfig:
    def __init__(self, **kwargs):
        ...
        # 损失权重
        self.rgb_loss_weight = kwargs.get('rgb_loss_weight', 1.0)
        self.dense_rgb_loss_weight = kwargs.get('dense_rgb_loss_weight', 3.0)
        self.smoothness_weight = kwargs.get('smoothness_weight', 0.1)
        self.smoothness_start_step = kwargs.get('smoothness_start_step', 300)
        self.mass_weight = kwargs.get('mass_weight', 0.05)
        self.mass_start_step = kwargs.get('mass_start_step', 200)
        self.monotonicity_weight = kwargs.get('monotonicity_weight', 0.001)
        self.monotonicity_start_step = kwargs.get('monotonicity_start_step', 200)
        # 空间 TV 损失（抗模糊）
...
class Trainer:
    ...
    def _render_rays(self, origins, directions, times):
        N = origins.shape[0]
    ...
    def _train_step(self, use_dense: bool):
        ...
        with autocast(device_type, enabled=self.config.mixed_precision):
            ...
            loss_rgb = F.mse_loss(attenuation, gt)
            loss_total = (self.config.dense_rgb_loss_weight if use_dense else self.config.rgb_loss_weight) * loss_rgb

            loss_smooth = torch.tensor(0.0, device=self.device)
            loss_mass = torch.tensor(0.0, device=self.device)
            loss_mono = torch.tensor(0.0, device=self.device)
            loss_tv = torch.tensor(0.0, device=self.device)

            if self.step >= self.config.smoothness_start_step and self.config.smoothness_weight > 0:
                loss_smooth = temporal_smoothness_loss(self.model, device=self.device)
                loss_total += self.config.smoothness_weight * loss_smooth

            if self.step >= self.config.mass_start_step and self.config.mass_weight > 0:
                loss_mass = mass_conservation_loss(self.model, device=self.device)
                loss_total += self.config.mass_weight * loss_mass
        ...
        while self.step < self.config.max_iterations:
            ...
            if self.step % self.config.print_every == 0:
                ...
                print(
                    ...
                    f"Smooth: {losses['smooth']:.6f} | Mass: {losses['mass']:.6f} | "
                    f"Mono: {losses['mono']:.6f} | TV: {losses['tv']:.6f} | LR: {lr:.2e}"
                )
...
def main():
    ...
    parser.add_argument('--num_samples_per_ray', type=int, default=128, help='每条光线的采样点数')
    ...
    parser.add_argument('--monotonicity_weight', type=float, default=0.001)
    parser.add_argument('--tv_weight', type=float, default=1e-5, help='空间总变差损失权重 (极轻量)')
    parser.add_argument('--resume', type=str, default=None)
    parser.add_argument('--output_dir', type=str, default='.')
    parser.add_argument('--no_4d_hash', action='store_true', help='回退到 MLP (无 tinycudann)')
    ...
    # 命令行覆盖
    if args.data_json: config.data_json = args.data_json
    ...
    config.mass_weight = args.mass_weight
